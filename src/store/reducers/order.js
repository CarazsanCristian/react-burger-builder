...
const purchaseBurgerSuccess=(state,action)=>{
    ...
    return updateObject(state,{loading:false, orders:state.orders.concat(newOrder), error: false, purchased:true});
};
const reducer = (state = initailState, action) => {
    switch (action.type) {
        case actionTypes.PURCHASE_BURGER_SUCCSESS:return purchaseBurgerSuccess(state,action);
        case actionTypes.PURCHASE_BURGER_FAIL:return updateObject(state,{loading:false,error: true});
